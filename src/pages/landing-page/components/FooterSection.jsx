    ...
    <footer className="bg-text-primary text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12 sm:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 sm:gap-12">
          ...
          <div className="sm:col-span-2 lg:col-span-2 space-y-4 sm:space-y-6">
            ...
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-primary rounded-lg flex items-center justify-center">
                <Icon name="Code2" size={20} color="white" className="sm:w-6 sm:h-6" />
              </div>
              <div>
                <div className="text-xl sm:text-2xl font-heading text-white">{t('footer.company.name')}</div>
                <div className="text-xs sm:text-sm text-white/60">{t('footer.company.tagline')}</div>
              </div>
            </div>
            ...
            <div className="space-y-2 sm:space-y-3">
              {t('footer.contactInfo', { returnObjects: true })?.map((contact, index) => (
                <div key={index} className="flex items-center space-x-3">
                  <Icon name={contact.icon} size={14} className="text-primary sm:w-4 sm:h-4" />
                  <div>
                    <div className="text-sm sm:text-base text-white/80">{contact.value}</div>
                    {contact.description && (
            ...
            <div className="flex items-center space-x-3 sm:space-x-4">
              {socialLinks?.map((social) => (
                <button
                  ...
                  className="w-8 h-8 sm:w-10 sm:h-10 bg-white/10 hover:bg-primary rounded-lg flex items-center justify-center transition-colors min-h-[44px] min-w-[44px]"
                  title={social?.name}
                >
                  <Icon name={social?.icon} size={16} className="sm:w-4 sm:h-4" />
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Services */}
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-white mb-4 sm:mb-6">{t('footer.sections.services')}</h3>
            <ul className="space-y-2 sm:space-y-3">
              {footerLinks?.services?.map((link) => (
                <li key={link?.label}>
                  <button
                    onClick={() => handleLinkClick(link?.href)}
                    className="text-white/70 hover:text-primary transition-colors text-xs sm:text-sm min-h-[32px]"
                  >
                    {link?.label}
                  </button>
                </li>
          ...
          {/* Company */}
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-white mb-4 sm:mb-6">{t('footer.sections.company')}</h3>
            <ul className="space-y-2 sm:space-y-3">
          ...
          {/* Support */}
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-white mb-4 sm:mb-6">{t('footer.sections.support')}</h3>
            <ul className="space-y-2 sm:space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
            {certifications?.map((cert) => (
              <div key={cert?.name} className="text-center">
                <div className="w-12 h-12 sm:w-16 sm:h-16 bg-white/10 rounded-lg flex items-center justify-center mx-auto mb-2 sm:mb-3">
                  <Icon name="Shield" size={20} className="text-primary sm:w-6 sm:h-6" />
                </div>
                <div className="text-white font-medium text-xs sm:text-sm">{cert?.name}</div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between space-y-3 sm:space-y-4 md:space-y-0">
            ...
            <div className="flex items-center space-x-4 sm:space-x-6">
              {footerLinks?.legal?.map((link) => (
                <button
                  key={link?.label}
                  onClick={() => handleLinkClick(link?.href)}
                  className="text-white/60 hover:text-primary transition-colors text-xs sm:text-sm min-h-[32px]"
                >
                  {link?.label}
                </button>
              ))}
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-white/60 text-xs sm:text-sm">{t('footer.status')}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-background border border-border rounded-2xl max-w-4xl w-full max-h-[80vh] overflow-hidden shadow-2xl">
            ...
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-border">
              ...
              >
                <Icon name="X" size={20} className="text-text-secondary sm:w-6 sm:h-6" />
              </button>
            </div>
            ...
            <div className="p-4 sm:p-6 overflow-y-auto max-h-[60vh]">
              <div 
                className="prose prose-sm max-w-none text-text-secondary text-sm sm:text-base"
                dangerouslySetInnerHTML={{ __html: modalContent.content }}
              />
            </div>

            {/* Modal Footer */}
            <div className="flex justify-end p-4 sm:p-6 border-t border-border">
              <button
                onClick={closeModal}
                className="px-4 sm:px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors text-sm sm:text-base min-h-[44px]"
              >
